...
const defaultWaTemplate = `Halo Bapak/Ibu {nama},\n\nIni adalah layanan SI-TAMU Lapas Kelas IIB Kalabahi.\nPendaftaran kunjungan virtual Anda untuk jadwal *{jadwal}* kepada Warga Binaan An. *{Nama Warga Binaan}* telah *{status}*.\n\nMohon bersiap, petugas akan segera menghubungi Anda melalui Video Call nomor ini.\nTerima Kasih.`;
...
export default function App() {
  ...
  const [waTemplate, setWaTemplate] = useState(() => {
    const saved = localStorage.getItem('sapaWaTemplate');
    return saved ? saved : defaultWaTemplate;
  });
  ...
  const handleSaveSettings = (e) => {
    ...
    localStorage.setItem('sapaWaTemplate', tempWaTemplate);
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0b1120] text-slate-950 dark:text-slate-100 transition-colors duration-500 font-sans selection:bg-emerald-500/30 overflow-x-hidden relative">
        ...
        <div className="flex gap-3 pointer-events-auto items-center">
          ...
          {adminUser && currentView !== 'public' && (
            <>
              ...
              <button onClick={() => setShowLogoutConfirm(true)} className="p-3.5 bg-rose-50 text-rose-600 border border-rose-200 dark:bg-rose-900/20 dark:border-rose-900/50 dark:text-rose-400 rounded-2xl hover:scale-105 hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-all shadow-sm" title="Keluar dari Dashboard">
                <LogOut size={18} />
              </button>
            </>
          )}
      ...
      <div className="pt-24 pb-12 px-6 min-h-screen relative z-10">
        ...
        <div className="transition-all duration-700 ease-in-out relative">
          {currentView === 'public' && <div className="animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-700"><PublicPortal db={db} appId={appId} showToast={showToast} firebaseUser={firebaseUser} dataKunjungan={dataKunjungan} masterWbp={masterWbp} /></div>}
          {currentView === 'admin-login' && <div className="animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-700"><AdminLogin onLogin={(u) => { setAdminUser(u); localStorage.setItem('sitabaAdmin', JSON.stringify(u)); setCurrentView('admin-dashboard'); showToast("Login Berhasil"); }} onBack={() => setCurrentView('public')} showToast={showToast} getAdminCreds={getAdminCreds} /></div>}
          {currentView === 'admin-dashboard' && <div className="animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-700"><AdminDashboard dataKunjungan={dataKunjungan} db={db} appId={appId} showToast={showToast} masterWbp={masterWbp} waTemplate={waTemplate} /></div>}
        </div>
              ...
              <form onSubmit={handleSaveSettings} className="space-y-6 text-left">
                ...
                <div className="space-y-2 border-t border-slate-200 dark:border-slate-800 pt-4">
                  <label className="text-[10px] font-black uppercase text-slate-500 ml-1 flex justify-between items-center">
                    ...
                    <span className="text-[9px] text-emerald-500 lowercase tracking-normal font-medium bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full">Gunakan: {'{nama}'}, {'{jadwal}'}, {'{Nama Warga Binaan}'}, {'{status}'}</span>
                  </label>
        ...
        {showLogoutConfirm && (
          <div className="fixed inset-0 z-[700] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="glass-card p-8 rounded-3xl w-full max-w-sm shadow-2xl relative text-center border border-rose-500/30">
              ...
              <h3 className="text-lg font-black mb-2 text-slate-900 dark:text-white">Keluar Dashboard?</h3>
              <p className="text-sm font-bold text-slate-500 mb-8">Anda harus login kembali untuk masuk ke dashboard admin.</p>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => { localStorage.removeItem('sitabaAdmin'); setAdminUser(null); setCurrentView('public'); showToast("Berhasil Keluar"); setShowLogoutConfirm(false); }} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-rose-700 transition-colors shadow-md shadow-rose-600/20">Ya, Keluar</button>
              </div>
            </div>
          </div>
        )}
...
function PublicPortal({ db, appId, showToast, firebaseUser, dataKunjungan, masterWbp }) {
  ...
  // FITUR: Kompresi Foto Otomatis (Ukuran KTP HP langsung diperkecil)
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      if (file.type === 'application/pdf') {
        if (file.size > 800000) return showToast("Ukuran PDF maksimal 800KB", "error");
        ...
        reader.onload = (event) => {
          ...
          img.onload = () => {
            ...
            setKtpFile(canvas.toDataURL('image/jpeg', 0.8)); // Kompres jadi kualitas 80%
          };
  ...
  const nextStep = () => {
    if (step === 1) {
      if (!formData.namaPengunjung || !formData.nik || !formData.noWa || !formData.hubungan || !formData.alamat || !ktpFile) {
        return showToast("Mohon lengkapi semua data diri & upload KTP", "error");
      }
      if (formData.nik.length !== 16) return showToast("NIK wajib 16 digit angka", "error");
      setStep(2);
    } else if (step === 2) {
      if (!formData.namaWbp) return showToast("Mohon pilih nama Warga Binaan", "error");
      setStep(3);
    }
  };
  ...
  const triggerSubmit = (e) => {
    e.preventDefault();
    if (!formData.tanggal || !formData.sesi) return showToast("Silakan pilih tanggal dan jam sesi", "error");
  ...
  if (successData) {
    return (
      <div className="max-w-md mx-auto mt-10 animate-in zoom-in duration-500">
        <div className="glass-card p-10 rounded-[3rem] text-center shadow-2xl border-emerald-500/50 relative overflow-hidden" id="ticket-area">
          ...
          <div className="mt-8 p-5 bg-amber-50/80 dark:bg-amber-900/10 rounded-3xl border border-amber-200/50 dark:border-amber-800/30" data-html2canvas-ignore="true">
  ...
  return (
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start min-h-[80vh]">
      {/* Kolom Kiri: Info */}
      ...
      <div className="lg:col-span-7 animate-in slide-in-from-right duration-1000 delay-150 relative">
        ...
        <div className="glass-card p-8 md:p-12 rounded-[3rem] shadow-2xl relative border border-slate-300 dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/60">
          ...
          <form onSubmit={triggerSubmit} className="space-y-0 relative min-h-[300px]">
            ...
            <div className="space-y-6 animate-in slide-in-from-left-4 fade-in duration-500">
                    ...
                    {ktpFile ? (
                      <div className="w-full relative h-48 bg-slate-100 dark:bg-slate-800 rounded-2xl overflow-hidden border border-emerald-500 flex items-center justify-center">
                        {ktpFile.startsWith('data:application/pdf') ? (
                          <iframe src={ktpFile} className="w-full h-full pointer-events-none border-none bg-white" title="Preview PDF" />
                        ) : (
                          <img src={ktpFile} className="w-full h-full object-contain pointer-events-none" alt="Preview KTP" />
                        )}
                        <div className="absolute inset-0 bg-emerald-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-2 backdrop-blur-sm">
                          <Upload size={24} />
                          <span className="text-[10px] font-black uppercase tracking-widest">Ganti File</span>
                        </div>
                      </div>
                    ) : (
                      <>
              ...
              <div className="flex justify-end pt-4">
                <button type="button" onClick={nextStep} className="px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[11px] tracking-widest btn-3d flex items-center gap-2">Selanjutnya <ChevronRight size={16}/></button>
              </div>
            </div>
            )}
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-500">
              ...
              <div className="flex justify-between pt-4">
                <button type="button" onClick={prevStep} className="px-8 py-4 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl font-black uppercase text-[11px] tracking-widest hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors flex items-center gap-2"><ChevronRight size={16} className="rotate-180"/> Kembali</button>
                <button type="button" onClick={nextStep} className="px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[11px] tracking-widest btn-3d flex items-center gap-2">Selanjutnya <ChevronRight size={16}/></button>
              </div>
            </div>
            )}
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-500">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select value={formData.sesi} onChange={e=>setFormData({...formData, sesi: e.target.value})} disabled={!formData.tanggal || availableSesi.length === 0} className="w-full px-5 py-4 premium-input rounded-2xl text-[11px] font-bold uppercase outline-none appearance-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-all text-slate-900 dark:text-white" required>
                      ...
                      {availableSesi.map(s => {
                        const sLabel = `${s} WITA`;
                        const kuotaTerpakai = dataKunjungan.filter(d => d.tanggal === formData.tanggal && d.sesi === sLabel && d.status !== 'Ditolak').length;
                        const isFull = kuotaTerpakai >= 10;
                        return (
                          <option key={s} value={sLabel} disabled={isFull}>
                            {sLabel} {isFull ? '(PENUH)' : `(Sisa ${10 - kuotaTerpakai} Slot)`}
                          </option>
                        );
                      })}
                    </select>
              ...
              <div className="flex flex-col sm:flex-row justify-between pt-4 gap-4">
                <button type="button" onClick={prevStep} disabled={loading} className="px-8 py-5 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl font-black uppercase text-[11px] tracking-widest hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-2 w-full sm:w-auto shrink-0"><ChevronRight size={16} className="rotate-180"/> Kembali</button>
                <button type="submit" disabled={loading} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] btn-3d-blue flex justify-center items-center gap-3">
                  {loading ? <Loader2 className="animate-spin" size={20}/> : <><Check size={18}/> Kirim Form Kunjungan Online</>}
                </button>
              </div>
            </div>
            )}
      ...
      {showConfirmSubmit && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm animate-in fade-in zoom-in-95 duration-200">
          <div className="glass-card p-8 rounded-3xl w-full max-w-sm shadow-2xl relative text-center border border-blue-500/30">
            <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle2 size={32} className="text-blue-600 dark:text-blue-500"/>
            </div>
            <h3 className="text-lg font-black mb-2 text-slate-900 dark:text-white">Konfirmasi Data</h3>
            <p className="text-xs font-bold text-slate-500 mb-8 leading-relaxed">Pastikan semua data dan jadwal sudah benar. Apakah Anda yakin ingin mengirim pendaftaran ini?</p>
            
            <div className="flex gap-3">
              <button onClick={() => setShowConfirmSubmit(false)} disabled={loading} className="flex-1 py-3 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">Batal</button>
              <button onClick={executeSubmit} disabled={loading} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-700 transition-colors shadow-md shadow-blue-600/20 flex items-center justify-center">
                {loading ? <Loader2 className="animate-spin" size={16}/> : 'Ya, Kirim'}
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center animate-in zoom-in duration-500">
      <div className="glass-card p-10 rounded-[3rem] w-full max-w-sm text-center border border-white/20 shadow-2xl relative overflow-hidden">
        ...
        <button onClick={onBack} className="absolute top-6 left-6 flex items-center gap-1.5 px-4 py-2 bg-slate-100 dark:bg-slate-800/50 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all font-bold text-[10px] uppercase tracking-widest border border-slate-200 dark:border-slate-700/50 shadow-sm z-20">
          <ChevronRight className="rotate-180" size={14} /> Kembali
        </button>

        <Shield size={50} className="text-blue-600 dark:text-blue-500 mx-auto mb-6 mt-10 drop-shadow-lg" />
        <h1 className="text-2xl font-black tracking-tighter dark:text-white leading-tight text-slate-900">Login Petugas</h1>
        <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 mt-2 mb-8">Akses Dashboard SI TAMU LAPAS KALABAHI</p>
...
// ==========================================
function AdminDashboard({ dataKunjungan, db, appId, showToast, masterWbp, waTemplate }) {
  ...
  useEffect(() => {
    const totalMenunggu = dataKunjungan.filter(d => d.status === 'Menunggu').length;
    if (totalMenunggu > prevMenungguRef.current && prevMenungguRef.current !== 0) {
      try {
        const audio = new Audio('https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3');
        audio.play().catch(e => console.log('Auto-play diblokir browser', e));
      } catch(e) {}
    }
    prevMenungguRef.current = totalMenunggu;
  }, [dataKunjungan]);

  useEffect(() => { 
    setSelectedKunjungan([]); 
    setCurrentPageKunjungan(1);
  }, [activeTab, adminView, searchTerm, filterTime]);
  ...
  const filteredWbpAll = masterWbp.filter(w => w.nama.toLowerCase().includes(searchWbp.toLowerCase()) || (w.status && w.status.toLowerCase().includes(searchWbp.toLowerCase())));
  const indexOfLastWbp = currentPageWbp * itemsPerPageWbp;
  const indexOfFirstWbp = indexOfLastWbp - itemsPerPageWbp;
  const filteredWbp = filteredWbpAll.slice(indexOfFirstWbp, indexOfLastWbp);
  const totalWbpPages = Math.ceil(filteredWbpAll.length / itemsPerPageWbp);
  ...
  const handleWA = (noWa, nama, jadwal, status, namaWbp) => {
    const formattedNum = formatPhoneWA(noWa);
    let text = waTemplate
      .replace(/{nama}/gi, nama)
      .replace(/{jadwal}/gi, jadwal)
      .replace(/{Nama Warga Binaan}/gi, namaWbp)
      .replace(/{status}/gi, status.toUpperCase());
  ...
  const filteredDataAll = dataKunjungan.filter(d => {
    ...
    if (filterTime === 'Harian') { matchDate = d.tanggal === todayStrExact; } 
    ...
    else if (filterTime === 'Bulanan') { matchDate = d.tanggal.startsWith(`${y}-${m}`); } 
  ...
  const indexOfLastKunjungan = currentPageKunjungan * itemsPerPage;
  const indexOfFirstKunjungan = indexOfLastKunjungan - itemsPerPage;
  const filteredData = filteredDataAll.slice(indexOfFirstKunjungan, indexOfLastKunjungan);
  const totalKunjunganPages = Math.ceil(filteredDataAll.length / itemsPerPage);

  const handleExportExcelKunjungan = () => {
    if (!window.XLSX) return showToast("Library Excel belum siap", "error");
    const data = filteredDataAll.map((d, i) => ({
  ...
  const handlePrintKunjungan = () => {
    ...
    let html = `
      <html><head><title>Laporan Kunjungan SI-TAMU KALABAHI</title>
      <style>
  ...
  const handleExportExcelWbp = () => {
    if (!window.XLSX) return showToast("Library Excel belum siap", "error");
    const data = filteredWbpAll.map((w, i) => ({ 
      'No': i + 1, 
      'Nama Lengkap': w.nama, 
  ...
  const stats = {
    ...
    selesai: dataKunjungan.filter(d => d.status === 'Selesai').length,
    ditolak: dataKunjungan.filter(d => d.status === 'Ditolak').length,
    total: dataKunjungan.length || 1
  };
